...
export default function ServiceDetailPage({ params }) {
  ...
  return (
    <>
        ...
        <div className="max-w-[1400px] mx-auto px-10 max-sm:px-[22px]">
          <Reveal>
            <div className="grid grid-cols-[200px_1fr] max-md:grid-cols-1 gap-12 max-w-[1100px]">
              ...
              <p className="text-[18px] text-titanium leading-[1.75]">
                {s.description}
              </p>
            </div>
          </Reveal>
            ...
            <div className="grid grid-cols-[200px_1fr] max-md:grid-cols-1 gap-12">
              ...
              <ol className="list-none counter-reset space-y-6 max-w-[820px]">
                {s.deliverables.map((d, i) => (
                  <li
                    key={i}
                    className="pl-14 relative text-[17px] text-titanium leading-[1.7] border-b border-line pb-6 last:border-0"
                  >
                    <span className="absolute left-0 top-1 font-mono text-[12px] tracking-extra-wide text-gold">
          ...
          <Reveal>
            <div className="grid grid-cols-[200px_1fr] max-md:grid-cols-1 gap-12">
              ...
              <p className="text-[17px] text-titanium leading-[1.75] max-w-[820px]">
                {s.process}
              </p>
            </div>
          </Reveal>
        ...
        <div className="max-w-[1400px] mx-auto px-10 max-sm:px-[22px]">
          <Reveal className="mb-16">
            ...
            <h3 className="font-display text-[clamp(36px,5vw,60px)] leading-[0.95] tracking-cinematic uppercase mt-7">
              OTHER <span className="text-gold">PRACTICES</span>
            </h3>
          </Reveal>
            ...
            {related.map((r, i) => (
              <Reveal key={r.slug} delay={i * 60}>
                <Link
                  ...
                  className="bg-obsidian-800 hover:bg-obsidian-700 p-10 max-sm:p-7 block transition-colors duration-300 h-full group"
                >
                  <div className="font-display text-3xl text-titanium-deep group-hover:text-gold transition-colors mb-5 tracking-cinematic">
                    {r.number}
                  </div>
                  <h4 className="font-display text-[24px] tracking-cinematic uppercase mb-3 text-white leading-[1]">
                    {r.title}
                  </h4>
                  <p className="text-[13px] text-titanium leading-[1.65]">
                    {r.tagline}
                  </p>
                </Link>
              </Reveal>
      ...
      <ContactCTA
        eyebrow={`ENGAGE THIS PRACTICE`}
        heading="START WITH"
        goldHeading={s.title.toUpperCase() + '.'}
        sub="Brief us on the situation. We will tell you whether this practice is the right entry point or whether the work calls for more."
      />
    </>
  );
}
