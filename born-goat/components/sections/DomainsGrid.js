      ...
      <div className="max-w-[1400px] mx-auto px-10 max-sm:px-[22px]">
        <Reveal className="mb-20 max-w-[920px]">
          ...
          <h2 className="font-display text-[clamp(48px,8vw,120px)] leading-[0.92] tracking-cinematic uppercase mt-7">
            WHERE WE <span className="text-sports-cyan">TRACK.</span>
          </h2>
          <p className="text-slate-300 text-[17px] leading-[1.7] max-w-[620px] mt-7">
            Eight sports verticals, complete real-time tracking metrics in each. We support domestic leagues, player stats, and prediction API models.
          </p>
        </Reveal>
        ...
        <div className="grid grid-cols-4 max-md:grid-cols-2 gap-px bg-line border border-line">
          {sportsDomains.map((d, i) => (
            <Reveal key={d.name} delay={i * 40}>
              <div className="bg-slate-900 hover:bg-slate-800 p-12 max-sm:px-6 text-center transition-colors duration-[400ms] cursor-default relative overflow-hidden group">
                ...
                <div className="w-12 h-12 mx-auto mb-5 text-slate-500 transition-all duration-[400ms] group-hover:text-sports-cyan group-hover:scale-110">
                  <svg
                    viewBox="0 0 48 48"
                    fill="none"
                    stroke="currentColor"
                ...
                <p className="text-[11px] text-slate-500 font-mono tracking-wide leading-[1.5]">
                  {d.sub}
                </p>
              </div>
            </Reveal>
